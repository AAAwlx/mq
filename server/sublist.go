...
type Partition struct {
	...
	key              string
	queue            []string
	consumer		 map[string]*Client//该分区的消费者
...
func (s *SubScription) ReduceConsumer(cli_name string){
	...
	switch s.option{
	case TOPIC_NIL_PTP: // 如果订阅选项是点对点模式（NIL_PTP）
		s.groups[0].DeleteClient(cli_name)
		s.consistent.Reduce(cli_name)
	case TOPIC_KEY_PSB:// 如果订阅选项是按键的发布/订阅模式（KEY_PSB）
		for _, group := range s.groups {
